...
import {
  ...
  hasBengaliText, rasterizeTextLine, loadBengaliCanvasFont,
} from "./utils.js";
...
async function exportLeaderboardPdf(mode, rows, usersMap, examId) {
  ...
  try {
    ...
    // result PDF already does. Falls back to native (unshaped) text only
    // if the canvas font truly can't load (e.g. offline).
    const canvasFont = await loadBengaliCanvasFont().catch(() => null);
